OWI.factory('StorageService', [ "DBService", function(DBService) {
  var service = {
    data: {},
    settings: {},
    defaultSettings: {
    ...
    init: function() {
      ...
      DBService.load_json().then(function(json) {
        console.log(json);
        var storedData = json; // localStorage.getItem('data');
...
OWI.factory("DataService", ["$http", "$q", "StorageService", "DBService", "$timeout", function($http, $q, StorageService, DBService, $timeout) {
...
OWI.factory('DBService', ["$http", function($http) {
  return {
    save_json: function(json) {
      console.log(json);
      return $http.post('/save-json', json)
        .then(function(res) { return res; });
    },
    load_json: function() {
      return $http.get('/load-json/' + location.pathname.substring(6))
        .then(function(res) {
